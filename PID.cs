...
namespace ksp
{
    public class PID
    {
        ...
        public PID(double Kp, double Ki, double Kd, double N, double OutputUpperLimit = 1, double OutputLowerLimit = 0)
        {
            this.Kp = Kp;
            this.Ki = Ki;
            this.Kd = Kd;
        ...
        // Overload for millisecond sample period
        public double PID_iterate(double setPoint, double processValue, int ts)
        {
            return PID_iterate(setPoint, processValue, TimeSpan.FromMilliseconds(ts));
        }
